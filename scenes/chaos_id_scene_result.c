...
void chaos_id_scene_result_on_enter(void* context) {
    ...
    if(!p) {
        widget_add_string_element(
            app->widget, 64, 32, AlignCenter, AlignCenter, FontPrimary, "No result");
        view_dispatcher_switch_to_view(app->view_dispatcher, ChaosIdViewWidget);
        return;
    }

    // Botao "Investigate" so aparece pra MIFARE Classic com risco Broken
    // (target card for the v0.5 dictionary attack)
    ...
    furi_string_cat_printf(dossier, "Data: %s\n", app->card_data_buffer);
    furi_string_cat_str(dossier, "\n");
    if(p->typical_use) {
        furi_string_cat_printf(dossier, "Use: %s\n", p->typical_use);
    }
    furi_string_cat_printf(dossier, "Crypto: %s\n", p->crypto ? p->crypto : "-");
    furi_string_cat_printf(dossier, "Attack: %s\n", p->attack_vector ? p->attack_vector : "-");
    if(p->year_broken && strcmp(p->year_broken, "-") != 0) {
        furi_string_cat_printf(dossier, "Broken: %s\n", p->year_broken);
    }
    furi_string_cat_printf(dossier, "Risk: %s\n", card_risk_label(p->risk));
    if(p->notes) {
        furi_string_cat_str(dossier, "\n");
    ...
    // Shrink scroll area when button is shown to avoid overlap
    ...
    if(can_attack) {
        widget_add_button_element(
            app->widget, GuiButtonTypeCenter, "Investigate", result_button_callback, app);
    }
